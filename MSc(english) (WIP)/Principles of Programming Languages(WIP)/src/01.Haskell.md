...
We see `reverse` takes one argument, a list. It then outputs a list of the same type. It’s a **pure function**, so it cannot change the input! Instead, it constructs a **completely new list** with the same elements as the input, only in reverse order. This may seem strange, but it has huge implications on our ability to reason about code.
...
Haskell uses lazy evaluation, which means that values are not computed until they are actually needed. This can have a big impact on the performance of your code, so you should understand the trade-offs involved.
