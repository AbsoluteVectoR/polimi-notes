...
The atomic directive enforces atomic access to a specific storage location mainly to assure the absence of problems caused by the concurrency write/read. Different aspects of the statements are performed atomically depending on the clauses: ```read``` , ```write``` , ```update``` are possible clauses. 

The OpenMP Data Scope Attribute Clauses are used to explicitly define how variables (shared by default) should be scoped. 

### Scope of variables

Scope variables in OpenMP are used to control the visibility and accessibility of variables within a parallel region of code. The use of scope variables is an essential part of writing effective parallel code with OpenMP, since they can help to improve the performance and efficiency of the parallel code.
